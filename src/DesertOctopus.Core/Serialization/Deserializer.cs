    ...
    internal static class Deserializer
    {
        ...
        internal static Func<ParameterExpression, ParameterExpression, Expression> GetPrimitiveReader(Type type)
        {
            ...
            if (type.GetTypeInfo().IsEnum
                && LazyPrimitiveMap.Value.TryGetValue(Enum.GetUnderlyingType(type),
                                                      out reader))
            {
                return reader;
            }
        ...
        private static System.Delegate CreateTypeDeserializer(Type type)
        {
            ...
            else if (typeof(ISerializable).GetTypeInfo().IsAssignableFrom(type.GetTypeInfo()) || options??)
            {
        ...
        internal static object ConvertObjectToIQueryable(object instance, Type expectedQueryableType)
        {
            ...
            Debug.Assert(typeof(IQueryable).GetTypeInfo().IsAssignableFrom(expectedQueryableType.GetTypeInfo()), "Type is not assignable");
