...
namespace DesertOctopus.Utilities
{
    ...
    internal static class ObjectCleaner
    {
        ...
        internal static object PrepareObjectForSerialization(object objToPrepare)
        {
            ...
            if (enumerableValue != null)
            {
                ...
                if (objectType.IsArray
                    || typeof(IList).GetTypeInfo().IsAssignableFrom(objectType.GetTypeInfo())
                    || typeof(ICollection).GetTypeInfo().IsAssignableFrom(objectType.GetTypeInfo()))
                {
                    return objToPrepare;
                }
                ...
                if (IsEnumeratingType(enumerableValue))
                {
                    ...
                    var enumerableInterface = objectType.GetTypeInfo().ImplementedInterfaces.FirstOrDefault(x => x.GetTypeInfo().IsGenericType && x.GetGenericTypeDefinition() == typeof(IEnumerable<>));
                    if (enumerableInterface != null)
                    {
                        itemType = enumerableInterface.GetTypeInfo().GenericTypeArguments[0];
                    }
        ...
        internal static object ConvertObjectToExpectedType(object obj, Type expectedType)
        {
            ...
            {
                var m = QueryableMih.AsQueryable(expectedType.GetTypeInfo().GenericTypeArguments[0]);
                return m.Invoke(null, new[] { obj });
            }

            if (typeof(IQueryable).GetTypeInfo().IsAssignableFrom(expectedType.GetTypeInfo()))
            {
