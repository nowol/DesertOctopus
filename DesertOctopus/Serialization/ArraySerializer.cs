    ...
    internal class ArraySerializer
    {
                                                              ...
                                                              ParameterExpression objTracking)
        {
            ...
            notTrackedExpressions.Add(Expression.IfThen(Expression.GreaterThanOrEqual(Expression.Constant(rank), Expression.Constant(255)),
                                                        Expression.Throw(Expression.New(typeof(NotSupportedException).GetConstructor(new[] { typeof(string) }), Expression.Constant("Array with more than 255 dimensions are not supported")))));
                                                                     ...
                                                                     ParameterExpression objTracking)
        {
                                                               ...
                                                               innerExpr) =>
            {
                ...
                loopExpressions.Add(Expression.Assign(item, Expression.Convert(Expression.Call(arr, typeof(Array).GetMethod("GetValue", new[] { typeof(int[]) }), indices), elementType)));
                                                                    ...
                                                                    int rank)
        {
            ...
            var length = Expression.Call(arr, typeof(Array).GetMethod("GetLength"), i);
